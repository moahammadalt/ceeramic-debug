...
import { CIP88_REF_PREFIX, CIP11_DEFINITION_SCHEMA_URL, CIP88_APPEND_COLLECTION_PREFIX } from '@glazed/constants';
import { model } from '@glazed/did-datastore-model';
...
function decodeDagJWS({ payload, signatures, link }) {
  return { payload, signatures, link: link ? new CID(link) : void 0 };
}
...
async function createModelDoc(ceramic, content, metadata = {}) {
  const doc = await TileDocument.create(ceramic, content, metadata, PUBLISH_OPTS);
  await ceramic.pin.add(doc.id);
  return doc;
}
async function publishCommits(ceramic, [genesis, ...updates]) {
  const doc = await TileDocument.createFromGenesis(ceramic, genesis, PUBLISH_OPTS);
  await ceramic.pin.add(doc.id);
...
const dataStoreModel = decodeModel(model);
...
const _ModelManager = class {
  ...
  async _loadAndValidateStream(id) {
    const stream = await TileDocument.load(__privateGet(this, _ceramic), id);
    ...
    const commits = await Promise.all(stream.allCommitIds.map(async (commitID) => {
      return await TileDocument.load(__privateGet(this, _ceramic), commitID);
    }));
  ...
  async createDefinition(alias, definition) {
    ...
    const [stream, schemaID] = await Promise.all([
      createModelDoc(__privateGet(this, _ceramic), definition, { schema: CIP11_DEFINITION_SCHEMA_URL }),
...
const validateSchemaSecure = new Ajv({ strict: false }).compile(SecureSchema);
...
export { ModelManager, addModelSchema, applyMap, createGraphQLModel, createModelDoc, decodeDagJWS, decodeDagJWSResult, decodeEntryCommits, decodeModel, decodeSignedMap, encodeDagJWS, encodeDagJWSResult, encodeEntryCommits, encodeModel, encodeSignedMap, extractSchemaReferences, getItemField, getReference, isSecureSchema, promiseMap, publishCommits, publishDataStoreSchemas, publishEncodedModel, publishModel, publishSignedMap, streamIDToString, validateSchemaSecure };
